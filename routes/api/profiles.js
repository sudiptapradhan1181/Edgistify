...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let profiles = await Profiles.findOneAndUpdate(
        { user: req.user.id },
        { $set: profileFields },
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const profiles = await Profiles.findOne({ user: req.user.id });
...
router.delete('/experience/:exp_id', auth, async (req, res) => {
  try {
    const foundProfiles = await Profiles.findOne({ user: req.user.id });
